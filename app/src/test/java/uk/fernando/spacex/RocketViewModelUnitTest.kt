...
class RocketViewModelUnitTest {
    ...
    fun `given success state, when getRockets called`() {
        ...
        coEvery { repository.getRockets() } returns Response.success(resource)
    ...
    fun `given error 404 not found, when getRockets called`() {
        ...
        //Given
        val bodyError = Response.error<List<RocketModel>?>(404, ResponseBody.create(MediaType.parse("type"), "test"))
        coEvery { repository.getRockets() } returns bodyError
